...
function prepare(stage, startIndex, size) {
    ...
    annotationFiles.slice(startIndex, startIndex + size).forEach(annotationFile => {
        ...
        let nonBgObjectIndex = annotation.objects.findIndex(object => object.classTitle != "bg");
        const annotationForVia = {
            fileref: "",
            class: annotation.objects[nonBgObjectIndex].classTitle,
            size: stats.size,
            ...
            regions: {
                "0": {
                    shape_attributes: {
                        name: "polygon",
                        all_points_x: annotation.objects[nonBgObjectIndex].points.exterior.map(point => point[0]),
                        all_points_y: annotation.objects[nonBgObjectIndex].points.exterior.map(point => point[1])
                    },
                    region_attributes: {}
                }
            }
